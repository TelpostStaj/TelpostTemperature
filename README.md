...
- **NVS'te "kayıt yok" ayrımı**: Kaydın varlığı `isKey()` ile sorulmalı, değerin kendisi nöbetçi (sentinel) olarak kullanılmamalıdır. Çeyrek yerleşimi tek bir `uint32` içinde paketlenir; paketin `0` olması geçerli bir yerleşimdir (dört çeyrekte de sıcaklık) ve "kayıt yok" ile karıştırılırsa kullanıcının seçimi sessizce varsayılana döner.
